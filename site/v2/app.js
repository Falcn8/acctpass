...
function createDownloadLink(download) {
  ...
  link.className = `button ${download.primary ? "button--dark" : "button--outline"}`;
  link.href = `${releaseBase}/${download.file}`;
  link.setAttribute("download", "");
  link.append(document.createTextNode(download.label), arrow);
...
function renderPlatform(key, { focus = false } = {}) {
  ...
  if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    ...
    const duration = Number.parseFloat(rootStyles.getPropertyValue("--dur-short")) || 220;
